def preciseOrbits2ECEF(sys, PRN, date_, dates, epochInterval, nEpochs, sat_positions, navGNSSsystems):
    ...
    # if date_ > tFirstEpoch:
    #     tk = (tFirstEpoch - date_).seconds + (tFirstEpoch - date_).microseconds/1e6
    # else:
    #     tk = (tFirstEpoch - date_).seconds + (tFirstEpoch - date_).microseconds/1e6 *-1
    # tk = timedelta(date_,tFirstEpoch)
    ...
    # nodeEpochs = list(range(node1EpochIndex,node8EpochIndex+1))
    ...
    for i in range(0,nNodes):
        ...
        # nodeTimes[i] = timedelta(dates[nodeEpochs[i], :], tFirstEpoch)
        ...
        # nodeTimes[i] = (date_dum - tFirstEpoch).seconds
    
    
    ## -- Interpolate new posistion of satellite using a lagrange polynomial
    # X=barylag(np.array([nodeTimes, nodePositions[:, 1]]), tk)
    # Y=barylag(np.array([nodeTimes, nodePositions[:, 2]]), tk)
    # Z=barylag(np.array([nodeTimes, nodePositions[:, 3]]), tk)
    try: 
