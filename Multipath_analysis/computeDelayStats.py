def computeDelayStats(ion_delay_phase1, multipath_range1, current_sat_elevation_angles, range1_slip_periods, LLI_slip_periods, range1_observations, tInterval):
    ...
    #--------------------------------------------------------------------------------------------------------------------------
    """
    from numpy import mean, sqrt, sin, nan,pi,isnan, sum, concatenate, zeros, where, intersect1d,nanmean,count_nonzero,union1d
    import warnings
    ...
    for i in range(0,nSat):
        ...
        slip_epochs = []
        # nSlipPeriods, _= range1_slip_periods[i+1].shape
       ...
       ## -- Get all slip epochs of current sat.
        ...
        for j in range(0,nSlipPeriods):
            ## if slip period is shorter than 60 seconds
            # slip_epochs = concatenate((slip_epochs, range1_slip_periods[i+1][j,0]))
        ...
        slip_epoch_elevation_angles = current_sat_elevation_angles[slip_epochs, i+1] ## tror det skal +1
        ...
        for j in range(0,nSlipPeriods):
            # LLI_slip_epochs = concatenate((LLI_slip_epochs, LLI_slip_periods[i+1][j,0])) + 1
