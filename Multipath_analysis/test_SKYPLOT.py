...
plt.rcParams['axes.facecolor'] = ((0.8731, 0.9515, 0.9868))
...
def make_skyplot(azimut_currentSys, elevation_currentSys, GNSSsystemName,graph_dir):
    ...
    # plt.title(title)
    ax.set_title("Skyplot for %s" % (GNSSsystemName), va='bottom',fontsize=30)
    for PRN in np.arange(0,num_sat):
        ...
        # ax.scatter(azimuth_rad, sat_el,label=PRN_,marker='')
