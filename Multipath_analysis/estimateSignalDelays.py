def estimateSignalDelays(range1_Code, range2_Code,phase1_Code, phase2_Code, carrier_freq1, \
    ...
    for epoch in range(0,nepochs):
        ...
        for i in range(0,n_sat):  
            ...
            range1 = GNSS_obs[epoch+1][PRN, ismember(obsCodes[currentGNSSsystem],range1_Code)] #epoch +1
            ...
            # if not any([range1, range2, phase1, phase2]) == 0:
    ...
    for PRN in range(0,max_sat):
        ...
        ## -- Run function to detect ambiguity slips for current epoch for range1/phase1 only 
        ...
        # Run function to detect ambiguity slips for current epoch for either range1/phase1 signal, range2/phase2 signal, or both
        ...
        if len(ambiguity_slip_periods[PRN]) == 0:
            if len(list(epoch_first_obs)) == 0:
                ...
                # ion_delay_phase1(epoch_first_obs:end, PRN) = ion_delay_phase1(epoch_first_obs:end, PRN) - ion_delay_phase1(epoch_first_obs, PRN)  
            ...
            #         mean(nonzeros(multipath_range2(epoch_first_obs:end, PRN)));
                multipath_range2[epoch_first_obs::, PRN] = multipath_range2[epoch_first_obs::, PRN] - np.mean(np.nonzero(multipath_range2[epoch_first_obs::, PRN])) ## ADD a if test like(1 if 2<2 else 2) to get rid of warning
        else:
            ...
            # for ambiguity_period in range(0,n_slip_periods + 1):
            for ambiguity_period in range(0,n_slip_periods+1): # removed + 1 cause of indexproblem 29.11
                ...
                if ambiguity_period == 0: 
                    ...
                    # ambiguity_period_end    = int(ambiguity_slip_periods[PRN][0,0]) -1# SJEKK opp tallen! 
                    ambiguity_period_end    = int(ambiguity_slip_periods[PRN][0,0])   #INK -1 igjen???
                ## -- If last ambiguity period
                # elif ambiguity_period == n_slip_periods + 1:  
    ...
    #                ambiguity_period_end         = epoch_last_obs; 
        ...
        # range1_observations = np.squeeze(GNSS_obs[:, ismember(obsCodes[currentGNSSsystem],range1_Code), :]) # MAALAB uses transpose her
        # phase1_observations = np.squeeze(GNSS_obs[:, ismember(obsCodes[currentGNSSsystem],phase1_Code), :]) # MAALAB uses transpose her
