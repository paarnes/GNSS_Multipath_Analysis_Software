...
def estimateSignalDelays(range1_Code, range2_Code,phase1_Code, phase2_Code, carrier_freq1, \
    ...
    --------------------------------------------------------------------------------------------------------------------------
     ...
     range1_Code:          matrix. Contains all range1 observations for all
                           epochs and for all SVs.

                           range1_observations(epoch, PRN)

     phase1:               matrix. Contains all phase1 observations for all
                           epochs and for all SVs.
    ...
    # ambiguity_slip_periods = {} # Initialize cell for storing phase slip periods
    ...
    missing_obs_overview = find_missing_observation(range1, range2, array3=phase1, array4=phase2) # Flag epoch and PRN as missing obs
    ...
    range1_slip_epochs = detectCycleSlips(N1_pseudo_estimate, missing_range1_overview, epoch_first_range1obs, epoch_last_range1obs, tInterval, phaseCodeLimit)
    # Run function to detect cycle slips for current epoch for either range1/phase1 signal, range2/phase2 signal, or both
    ionosphere_slip_epochs = detectCycleSlips(ion_delay_phase1, missing_obs_overview, epoch_first_obs, epoch_last_obs, tInterval, ionLimit)
    ...
    for PRN in np.arange(0,max_sat):
        ...
        ambiguity_slip_epochs = np.union1d(np.array(range1_slip_epochs[str(PRN)]),np.array(ionosphere_slip_epochs[str(PRN)]))
        ## -- Organize slips detected on range1/phase1 signal only
        # range1_slip_periods[PRN],_ = orgSlipEpochs(range1_slip_epochs)
        range1_slip_periods[int(PRN)],_ = orgSlipEpochs(np.array(range1_slip_epochs[str(PRN)]))

        ## -- Orginize combined slips detected on range1/phase1 signal only
        ambiguity_slip_periods[int(PRN)], n_slip_periods = orgSlipEpochs(ambiguity_slip_epochs)
        ...
        # range1_observations =  np.zeros([nepochs, max_sat+1]) ## DETTE KAN GJØRES MYE KJAPPPERE!!! DET ER DENNE SOM TAR MYYYYE TID!
